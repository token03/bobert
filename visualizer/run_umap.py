...
def process():
    ...
    # 2. Load Metadata
    meta_df = pd.read_parquet(BEATMAPS_PATH, columns=[
        "id", "title", "artist", "creator", "difficulty_rating", "status", "version"
    ])
    ...
    index = NNDescent(matrix, metric="cosine", n_neighbors=n_neighbors, n_jobs=-1, random_state=42)
    ...
    ui_indices, _ = index.query(matrix, k=7) # k=1 (self) + 6 neighbors
