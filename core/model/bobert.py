...
class BobertMaskedLMHead(nn.Module):
    ...
    def forward(
        ...
        continuous_preds = torch.zeros(
            batch_size,
            seq_len,
            continuous_preds_masked.shape[-1],
            device=device,
            dtype=continuous_preds_masked.dtype,
        )
        categorical_preds = {
            name: torch.zeros(
                batch_size, seq_len, preds.shape[-1], device=device, dtype=preds.dtype
            )
        ...
        for name in categorical_preds:
            categorical_preds[name][batch_indices, seq_indices] = (
                categorical_preds_masked[name]
            )
