...
def deduplicate_collections(df, threshold, probe_items=32):
    ...
    sorted_ckeys = sorted(unique_content_ckeys, key=lambda x: len(col_groups[x]), reverse=True)
    ...
    for ckey in tqdm(sorted_ckeys, desc="Deduplicating"):
        ...
        is_duplicate = False
        if overlap_counts:
            candidates = sorted(overlap_counts.items(), key=lambda kv: kv[1], reverse=True)[:5]
            for kept_ckey, _ in candidates:
                B = kept_sets[kept_ckey]
                len_b = len(B)
                
                if len_a < threshold * len_b or len_b < threshold * len_a: continue
...
def load_and_process_data(source_filter=None):
    ...
    if os.path.exists(COLLECTION_FILTER_PATH):
        ...
        if "collections" in filter_data:
            bad_ids = []
            for src, ids in filter_data["collections"].items(): bad_ids.extend(ids)
            df = df[~df["collection_id"].isin(bad_ids)]

    beatmaps_df = pd.read_parquet(BEATMAPS_PATH, columns=["id", "beatmapset_id", "mode", "status"])
    df = df.merge(beatmaps_df.rename(columns={"id": "beatmap_id"}), on="beatmap_id", how="left")
    
    df = df[df["mode"] == "osu"].copy()
    
    col_counts = df.groupby("collection_id")["beatmap_id"].count()
    valid_cols = col_counts[(col_counts >= MIN_MAPS_IN_COLLECTION) & (col_counts <= MAX_MAPS_IN_COLLECTION)].index
...
def compute_normalized_laplacian(user_indices, item_indices, values, num_users, num_items):
    ...
    return torch.sparse_coo_tensor(indices, norm_vals, (num_nodes, num_nodes)).coalesce()
...
class LightGCN(nn.Module):
    ...
    def forward(self, sparse_adj):
        with torch.amp.autocast('cuda', enabled=False):
            ego_embeddings = torch.cat([self.user_embedding.weight, self.item_embedding.weight], dim=0).float()
...
def train(source_filter=None):
    ...
    df, unique_collections, unique_beatmaps, col_to_idx, bm_to_idx = load_and_process_data(source_filter)

    num_users = len(unique_collections)
    num_items = len(unique_beatmaps)
    print(f"Graph: {num_users} Cols, {num_items} Maps, {len(df)} Edges")

    user_indices = torch.tensor(df["collection_key"].map(col_to_idx).values, dtype=torch.long, device=DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        for i in pbar:
            ...
            batch_neg = torch.randint(0, num_items, (len(idx),), device=DEVICE) + num_users
            ...
            with torch.amp.autocast('cuda'):
                ...
                reg_loss = (1/2) * (u_emb.norm(2).pow(2) + p_emb.norm(2).pow(2) + n_emb.norm(2).pow(2)) / float(len(idx))
    ...
    pd.DataFrame([{
        "collection_id": k[0], "source": k[1], "embedding": user_emb_np[i].tolist()
    } for k, i in col_to_idx.items()]).to_parquet(COLLECTIONS_DIR / f"collection_embeddings_{VERSION}.parquet")

    pd.DataFrame([{
        "beatmap_id": k, "embedding": item_emb_np[i].tolist()
    } for k, i in bm_to_idx.items()]).to_parquet(COLLECTIONS_DIR / f"beatmap_embeddings_{VERSION}.parquet")
