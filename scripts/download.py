...
def build_index_multithreaded():
    """
    Builds a comprehensive index of BeatmapIDs and their corresponding .osu file paths
    in the SONGS_FOLDER_PATH, using multiple threads for speed.
    """
...
def load_or_build_index(cache_dir):
    """
    Loads the beatmap index from cache if it's up-to-date, otherwise rebuilds it.
    The cache file is stored in the specified `cache_dir`.
    """
...
def process_file(input_file_path, beatmap_index):
    print(f"\n--- Processing {os.path.basename(input_file_path)} ---")
    
    output_folder = os.path.splitext(input_file_path)[0]
    ...
    if already_exist_ids: print(f"Already existed in output folder: {len(already_exist_ids)} maps")
    if found_locally_ids: print(f"Copied from local Songs folder: {len(found_locally_ids)} maps")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python download.py <path_to_input_txt_file>")
        print("Example: python download.py my_beatmaps.txt")
        sys.exit(1)
    ...
    process_file(str(input_file_path), beatmap_index)
